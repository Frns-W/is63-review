        ...
        <div class="card-body">
                        ...
                        @forelse($prodis as $prodi)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('prodi.edit', $prodi) }}" class="btn btn-warning btn-sm" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    <form id="form-hapus-{{ $prodi->id }}" action="{{ route('prodi.destroy', $prodi) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm" title="Hapus" onclick="return confirm('Apakah yakin menghapus data ini?');">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
                <small class="text-muted">
                    Menampilkan {{ $prodis->firstItem() }}–{{ $prodis->lastItem() }} dari {{ $prodis->total() }} data
                </small>
                {{ $prodis->links() }}
            </div>
        </div>
